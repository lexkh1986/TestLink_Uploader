...
class Connection(Test):
    ...
    def pullTestCases(self):
        ...
        if iTC_List:
            ...
            for iTC in iTC_List:
                iTC_Asignee = self.CONN.getTestCaseAssignedTester(testcaseexternalid = iTC[0]['full_external_id'],
                                                                  testplanid = self.TESTPLAN_ID,
                                                                  buildid = self.TESTBUILD_ID)
                if iTC_Asignee[0]['login'] in ('', None): continue # Only get which testcase belong to selected build
